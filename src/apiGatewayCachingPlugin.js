...
class ApiGatewayCachingPlugin {
  ...
  updateCloudFormationTemplate() {
    ...
    if (!this.thereIsARestApi) {
      this.serverless.cli.log(`[serverless-api-gateway-caching][before:package:finalize] No Rest API found. Caching settings will not be updated.`);
      return;
    }
  ...
  updateStage() {
    this.thereIsARestApi = restApiExists(this.serverless);
    this.serverless.cli.log(`[serverless-api-gateway-caching][after:aws:deploy:finalize:cleanup] rest api existance ${this.thereIsARestApi}.`);
    if (!this.thereIsARestApi) {
      this.serverless.cli.log(`[serverless-api-gateway-caching][after:aws:deploy:finalize:cleanup] No Rest API found. Caching settings will not be updated.`);
      return;
    }
